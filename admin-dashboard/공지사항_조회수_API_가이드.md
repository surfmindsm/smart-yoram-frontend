...
## 개요
공지사항에 조회수(view_count) 기능이 추가되었습니다.

**🎉 모바일 앱 코드 수정 불필요!**
- 기존 API를 그대로 사용하면 자동으로 조회수가 증가됩니다.
- 백엔드에서 자동으로 처리되므로 모바일 개발자는 아무것도 하지 않아도 됩니다.
...
### 모바일 개발자가 해야 할 일
1. **모델 업데이트**: `view_count` 필드 추가 (이미 응답에 포함됨)
2. **API 호출**: 기존 API 그대로 사용 (`GET /announcements/{id}`)
3. **조회수 증가**: 자동으로 처리됨 - **추가 코드 불필요** ✅

### 작동 방식
```
모바일 앱 → GET /announcements/{id} 호출
    ↓
백엔드 Edge Function
    ├─ view_count + 1 자동 증가
    └─ 증가된 데이터 반환
    ↓
모바일 앱 ← view_count 포함된 응답 수신
```
...
**엔드포인트:** `GET /announcements/church/{church_id}`
...
### 2. 공지사항 상세 조회 API - **조회수 자동 증가**

**엔드포인트:** `GET /announcements/{id}`

**중요:** 이 API를 호출하면 **자동으로 조회수가 1 증가**됩니다.
모바일에서는 별도의 조회수 증가 로직을 구현할 필요가 없습니다.
...
백엔드 Edge Function에서 자동으로 처리됩니다:

1. 공지사항 조회 요청 수신
2. **자동으로** `view_count` 필드를 +1 증가
3. 증가된 데이터를 응답으로 반환

### 2. ~~공지사항 상세 조회 API - **조회수 증가 필요**~~ (구버전 - 사용 안 함)
...
- [ ] 모델에 `view_count` 필드 추가 (응답에 이미 포함되어 있음)
- [ ] ~~공지사항 상세 조회 시 조회수 증가 API 호출~~ **불필요 - 자동으로 처리됨** ✅
- [ ] ~~중복 조회 방지 로직 구현~~ **불필요 - 백엔드에서 처리 가능** ✅
- [ ] UI에 조회수 표시 (선택사항)
- [ ] 기존 API 정상 작동 확인
